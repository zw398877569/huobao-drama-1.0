...
- **character_ids 必填规则 (Batch A #3, 2026-09-22)**: 画面里出现任何角色(包括背景人影、远处轮廓),
  character_ids 必须用数字 ID 列出 (例 形如 15 与 17, LLM 用 JSON 数组输出, 不要套 object).
  仅"纯环境空镜"(画面无任何角色/肢体/身影, 仅建筑/风景)才能用空数组.
  不允许用空数组偷懒 (豆豆在画面角落也算有角色, 必须列出 17)
